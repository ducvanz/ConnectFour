...
class MonteCarloTreeSearch :
    ...
    def select(self, game:ConnectFourBoard, counts:dict, wins:dict, losses:dict):
        ...
        for k in game.get_available_columns():
            ...
            if counts.get(k, 0) == 0 :
                ...
                scores[k] = (wins.get(k, 0) - losses.get(k, 0)) / counts[k] + \
                    self.temperature * sqrt(log(sum(counts.values())) / counts[k])
    ...
    def simulate(self, game_clone:ConnectFourBoard):
    ...
    def get_move(self, game:ConnectFourBoard):
        ...
        while rollouts_completed < self.num_rollouts and (time.time() - start_time) < self.time_limit :
            ...
            move = self.select(game, counts, wins, losses)
            ...
            reward = self.simulate(game_clone)
