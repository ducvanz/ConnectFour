...
def get_move(game, max_time=1.0):
    ...
    for i in range(game.columns):
        if game.board[0][i] != 0:  # Skip already full columns
            continue
    ...
    w3 = []
    first_move_scores = {i: 0 for i in valid_columns}
    ...
    for i in valid_columns:
        ...
        try:
            game_clone = deepcopy(game)
            if game_clone.drop_piece(i):  # This changes turn to opponent
                opponent_valid_columns = [c for c in range(game.columns) if game_clone.board[0][c] == 0]
                ...
                for j in opponent_valid_columns:
                    ...
                    if game_clone2.drop_piece(j):  # This changes turn back to original player
                        player_valid_columns = [c for c in range(game.columns) if game_clone2.board[0][c] == 0]
    ...
    if any(first_move_scores.values()):
        return max(first_move_scores, key=first_move_scores.get)
